...
import { ExecutionModeLib } from "../libraries/ExecutionModeLib.sol";
 ...
 */
abstract contract EIP7702DeleGatorCore is ExecutionHelper, IERC165, IDeleGatorCore, IERC721Receiver, IERC1155Receiver, EIP712 {
    ...
    function execute(ModeCode _mode, bytes calldata _executionCalldata) external payable onlyEntryPointOrSelf {
        if (!supportsExecutionMode(_mode)) revert UnsupportedExecutionMode(_mode);
     ...
     */
    function supportsExecutionMode(ModeCode _mode) public view virtual returns (bool result_) {
        return ExecutionModeLib._supportsExecutionMode(_mode);
    }
