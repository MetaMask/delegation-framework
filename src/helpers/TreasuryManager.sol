 ...
 *
 *      Multichain stETH support: this contract is meant to be deployed across multiple EVM chains from one source.
 *      `stEth` and `wstEth` are immutables that may be set to `address(0)` on chains where Lido stETH does not exist
 *      (everything except Ethereum mainnet). On those chains `wrapStEth` reverts with `WrappingNotSupported`; all
...
contract TreasuryManager is Ownable2Step, ReentrancyGuard {
    ...
    /// @notice Lido stETH address on chains where it exists; `address(0)` to disable `wrapStEth` (non-mainnet chains).
    address public immutable stEth;
    /// @notice Lido wstETH address on chains where it exists; `address(0)` to disable `wrapStEth` (non-mainnet chains).
    ...
    event ApiSignerUpdated(address indexed newSigner);
    ...
    event StEthWrapped(address indexed destWalletAddress, uint256 stEthIn, uint256 wstEthOut);
     ...
     * @param _stEth Lido stETH on chains where it exists; pass `address(0)` on chains without Lido (disables `wrapStEth`).
     * @param _wstEth Lido wstETH on chains where it exists; pass `address(0)` on chains without Lido (disables `wrapStEth`).
     */
    constructor(
        address _owner,
        address _apiSigner,
        ...
        nonReentrant
    {
        ...
        _swapTokens(aggregatorId_, tokenFrom_, tokenTo_, _signatureData.destWalletAddress, amountFrom_, swapData_);
    }
    ...
    /**
     * @notice Redeems `_stEthAmount` of stETH from the delegation, wraps it to wstETH, and forwards the resulting
     *         wstETH to `_destWalletAddress`.
     * @dev Only meaningful on chains where `stEth` and `wstEth` are non-zero (Ethereum mainnet today). On any other
     *      chain reverts with `WrappingNotSupported`. Plain `transfer` still uses strict equality and is unsuitable for
     *      stETH due to per-transfer share rounding; use this function instead when forwarding stETH-denominated value.
     *      Accepts up to a 2-wei shortfall on the redeemed stETH amount (intrinsic to Lido stETH share rounding); the
     *      wrapped wstETH amount delivered downstream is the actual minted balance, not the requested input amount.
     * @param _delegations Delegation chain authorizing a stETH pull of `_stEthAmount` into this contract.
     * @param _stEthAmount API/quote stETH amount the delegation will be asked to release.
     * @param _destWalletAddress Allowlisted recipient that will receive the wstETH minted by the wrap.
     */
    function wrapStEth(
        Delegation[] calldata _delegations,
        ...
        nonReentrant
    {
        if (_delegations.length == 0) revert InvalidEmptyDelegations();
        if (!isDestWalletAllowed[_destWalletAddress]) revert DestinationWalletNotAllowed();
        if (stEth == address(0) || wstEth == address(0)) revert WrappingNotSupported();

        // stETH share-rounding tolerance: accept up to 2 wei short. Hardcoded because this magic number is intrinsic
        // to Lido stETH's per-transfer rounding and is used nowhere else. Underflow on `_stEthAmount - 2` is caught
        // by Solidity's panic for any wrap below 3 wei (a degenerate input the wstETH contract would also reject).
        IERC20 wstEthToken_ = IERC20(wstEth);
        IERC20 stEthToken_ = IERC20(stEth);

        uint256 stEthObtained_ = _redeemTransfer(_delegations, stEthToken_, _stEthAmount, _stEthAmount - 2);

        _etherValueOrApprove(stEthToken_, stEthObtained_, wstEth);

        uint256 wstEthBefore_ = wstEthToken_.balanceOf(address(this));
        IWstETH(wstEth).wrap(stEthObtained_);
        uint256 wstEthMinted_ = wstEthToken_.balanceOf(address(this)) - wstEthBefore_;

        _sendTokens(wstEthToken_, wstEthMinted_, _destWalletAddress);
        emit StEthWrapped(_destWalletAddress, stEthObtained_, wstEthMinted_);
    }
    ...
    /// @dev Sets `apiSigner` and emits `ApiSignerUpdated`. Used by both the constructor and `setApiSigner`.
    function _setApiSigner(address _newSigner) private {
        _requireNonZero(_newSigner);
        apiSigner = _newSigner;
        emit ApiSignerUpdated(_newSigner);
    }
     ...
     *      into this contract (or native receive), then validates the actual balance delta and returns it. The accepted
     *      range is `[_amountExpected, _amount]` inclusive: the upper bound catches over-credit (defensive) and the
     *      lower bound is set by the caller to permit known per-transfer rounding shortfalls (e.g. stETH share
     *      rounding). Strict callers pass `_amountExpected == _amount`.
     ...
     * @param _amount Amount the redemption execution will request from the delegator (also the upper bound on the
     *                accepted balance delta).
     * @param _amountExpected Lower bound on the accepted balance delta. Pass `_amount` for strict equality, or
     *                        `_amount - tolerance` to allow a known shortfall.
     * @return obtained_ Balance delta this contract observed after the redemption.
     */
        ...
        returns (uint256 obtained_)
    {
        ...
        if (obtained_ < _amountExpected || obtained_ > _amount) revert UnexpectedTokenFromAmount(_amountExpected, obtained_);
    }
     ...
     *      full output to `_destWalletAddress`.
     */
        ...
        private
    {
        ...
        uint256 obtainedOut_ = _getSelfBalance(_tokenTo) - balanceToBefore_;
