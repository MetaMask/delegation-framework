...
/**
 * @title ERC721MaxLossEnforcer
 * @dev This contract enforces that the ERC721 token balance of a recipient has decreased at most the specified amount
 * after the execution, measured between the `beforeHook` and `afterHook` calls, regardless of what the execution is.
 */
contract ERC721MaxLossEnforcer is CaveatEnforcer {
    ////////////////////////////// State //////////////////////////////
        ...
        override
    {
        ...
        require(!isLocked[hashKey_], "ERC721MaxLossEnforcer:enforcer-is-locked");
    ...
    /**
     * @notice This function enforces that the delegator's ERC721 token balance has decreased at most the amount provided.
     * @param _terms 72 bytes where:
     * - first 20 bytes: address of the ERC721 token,
     * - next 20 bytes: address of the recipient,
        ...
        override
    {
        ...
        uint256 balance_ = IERC721(token_).balanceOf(recipient_);
        require(balance_ >= balanceCache[hashKey_] - amount_, "ERC721MaxLossEnforcer:balance-not-gt");
    }
    ...
    function getTermsInfo(bytes calldata _terms) public pure returns (address token_, address recipient_, uint256 amount_) {
        require(_terms.length == 72, "ERC721MaxLossEnforcer:invalid-terms-length");
