...
/**
 * @title Limited Calls by Redeemer Enforcer Contract
 * @dev This contract extends the CaveatEnforcer contract. It provides functionality to enforce a limit on the number of times
 * a redeemer may perform transactions on behalf of the delegator.
 */
contract LimitedCallsByRedeemerEnforcer is CaveatEnforcer {
    ////////////////////////////// State //////////////////////////////

    mapping(address delegationManager => mapping(bytes32 delegationHash => mapping(address redeemer => uint256 count))) public
        callCounts;
        ...
        override
    {
        ...
        uint256 callCounts_ = ++callCounts[msg.sender][_delegationHash][_redeemer];
        require(callCounts_ <= limit_, "LimitedCallsByRedeemerEnforcer:limit-exceeded");
    ...
    function getTermsInfo(bytes calldata _terms) public pure returns (uint256 limit_) {
        require(_terms.length == 32, "LimitedCallsByRedeemerEnforcer:invalid-terms-length");
