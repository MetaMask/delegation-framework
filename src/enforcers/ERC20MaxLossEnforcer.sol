...
contract ERC20MaxLossEnforcer is CaveatEnforcer {
     ...
     * is the address of the token, the next 32 bytes is the amount of tokens the balance can decrease by
     */
    function beforeHook(
    ...
    /**
     * @notice This function enforces that the delegators ERC20 balance has decreased by at most the amount provided.
     * @param _terms 72 packed bytes where: the first 20 bytes is the address of the recipient, the next 20 bytes
     * is the address of the token, the next 32 bytes is the amount the balance can decrease by
     */
    function afterHook(
        bytes calldata _terms,
        bytes calldata,
