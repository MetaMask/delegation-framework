...
/**
 * @title DelegationManagerC5
 * @notice Observable-semantics ablation C5: lean redemption events (hash-only, fully indexed).
 * @dev Single change vs canonical: emits `RedeemedDelegationLean` with `(rootDelegator, redeemer, delegationHash)` instead
 *      of the canonical `RedeemedDelegation` carrying the full `Delegation` struct in log data. Execution and hook
 *      behavior are unchanged; indexers must recover delegation fields from signed payloads off-chain.
 */
contract DelegationManagerC5 is IDelegationManager, Ownable2Step, Pausable, EIP712 {
    ...
    /// @dev Lean event variant — observable delta vs canonical `RedeemedDelegation`.
    event RedeemedDelegationLean(address indexed rootDelegator, address indexed redeemer, bytes32 indexed delegationHash);
