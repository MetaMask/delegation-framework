...
abstract contract DeleGatorTestSuite is BaseTest {
    ...
    /// Hook: Return the revert bytes for unsupported "call type" mode.
    function encodeUnsupportedCallTypeRevertReason(ModeCode mode_) internal virtual returns (bytes memory);

    /// Hook: Return the revert bytes for unsupported "exec type" mode.
    function encodeUnsupportedExecTypeRevertReason(ModeCode mode_) internal virtual returns (bytes memory);
    ...
    function test_notAllow_unsupportedCallType_UserOp() public {
        ...
        bytes memory revertReason_ = encodeUnsupportedCallTypeRevertReason(mode_);
        // Expect a revert event
    ...
    function test_notAllow_singleUnsupportedExecType_UserOp() public {
        ...
        bytes memory revertReason_ = encodeUnsupportedExecTypeRevertReason(mode_);
        // Expect a revert event
        vm.expectEmit(false, false, false, true, address(entryPoint));
        emit UserOperationRevertReason(userOpHash_, address(users.alice.deleGator), 0, revertReason_);
        submitUserOp_Bundler(userOp_, false);
...
abstract contract UUPSDeleGatorTest is DeleGatorTestSuite {
    ...
    function encodeUnsupportedCallTypeRevertReason(ModeCode /*mode_*/ ) internal pure override returns (bytes memory) {
        return abi.encodeWithSelector(DeleGatorCore.UnsupportedCallType.selector, CallType.wrap(0x02));
    }

    function encodeUnsupportedExecTypeRevertReason(ModeCode /*mode_*/ ) internal pure override returns (bytes memory) {
        return abi.encodeWithSelector(DeleGatorCore.UnsupportedExecType.selector, ExecType.wrap(0x02));
    }
...
abstract contract EIP7702DeleGatorTest is DeleGatorTestSuite {
    ...
    function encodeUnsupportedCallTypeRevertReason(ModeCode mode_) internal pure override returns (bytes memory) {
        return abi.encodeWithSelector(EIP7702DeleGatorCore.UnsupportedExecutionMode.selector, mode_);
    }
