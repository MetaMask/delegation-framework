...
contract EIP7702StatelessDeleGatorTest is BaseTest {
    ...
    function test_supportsExecutionMode() public {
        // 4 official modes from the library:
        // single revert -> 0
        // single try -> 1 << 240
        // batch revert -> 1 << 248
        // batch try -> (1 << 240) | (1 << 248)

        ModeCode singleRevertMode_ = ModeCode.wrap(0);
        ModeCode singleTryMode_ = ModeCode.wrap(bytes32(uint256(1 << 240)));
        ModeCode batchRevertMode_ = ModeCode.wrap(bytes32(uint256(1 << 248)));
        ModeCode batchTryMode_ = ModeCode.wrap(bytes32(uint256((1 << 240) | (1 << 248))));
    ...
    // Test revert if mode is unsupported -> UnsupportedExecutionMode
    function test_execute_revertIfUnsupportedExecutionMode() public {
        // We create a random "unsupported" ModeCode, e.g. 12345
        ModeCode unsupportedMode_ = ModeCode.wrap(bytes32(uint256(12345)));
        bytes memory executionCalldata_ = abi.encode(address(0), 0, bytes(""));

        // Emulate call from the EntryPoint or Self (otherwise we'd get a different revert).
        vm.prank(address(entryPoint));
        vm.expectRevert(abi.encodeWithSelector(EIP7702DeleGatorCore.UnsupportedExecutionMode.selector, unsupportedMode_));
        aliceDeleGator.execute(unsupportedMode_, executionCalldata_);
    }
}
