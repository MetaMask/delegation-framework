...
contract BalancePaymentTest is CaveatEnforcerBaseTest {
    ...
    /// This test ensures the enforcer properly protects against failed or incomplete swaps
    ...
    /**
     * @notice Tests state sharing vulnerability in ERC20BalanceChangeEnforcer when batching delegations
     ...
     */
    function test_batchingRedemptionsEnforcerShareState() public {
        // Create delegation from Alice to SwapMock allowing transfer of 1 ETH worth of TokenA
        bytes memory transferTerms_ = abi.encodePacked(address(tokenA), uint256(1 ether));
        bytes memory balanceTerms_ = abi.encodePacked(false, address(tokenB), address(delegator), uint256(2 ether));
        ...
        swapMock.swapDoubleSpend(delegations_, 1 ether);

        assertEq(tokenA.balanceOf(address(delegator)), 98 ether, "TokenA balance of delegator should be 98 ether");
        assertEq(tokenB.balanceOf(address(delegator)), 1 ether, "TokenB balance of delegator should be 1 ether");
        assertEq(tokenA.balanceOf(address(swapMock)), 2 ether, "TokenA balance of swapMock should be 2 ether");
        assertEq(tokenB.balanceOf(address(swapMock)), 99 ether, "TokenB balance of swapMock should be 99 ether");
    }
...
contract SwapMock is ExecutionHelper {
    ...
    // This contract swaps X amount of tokensIn for amount of tokensOut
    function swapDoubleSpend(Delegation[][] memory _delegations, uint256 _amountIn) external {
        ...
        executionCallDatas_[length_] = ExecutionLib.encodeSingle(
            address(this),
            0,
            abi.encodeWithSelector(this.validateAndTransfer.selector, _delegations[0][0].delegator, length_, _amountIn)
        );
     ...
     */
    function validateAndTransfer(address _recipient, uint256 _delegationsLength, uint256 _amountIn) external onlySelf {
        // Some condition representing the need for the ERC20 tokensIn at this point
        uint256 balanceTokenIn_ = tokenIn.balanceOf(address(this));
